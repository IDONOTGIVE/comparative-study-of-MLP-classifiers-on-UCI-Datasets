%This is the command for "myBackpropagation_Batch"
clear;
...
for n=1:runningcycles   
	...
	%train the data 
    [Wh,Wo] = myBackpropagation_Batch(Input(:,train),Target(:,train),[100,7,0.01,0.015]);
    
	%test the data
	result=Backpropagation_fwd(Wh,Wo,Input(:,train));
end
time=cputime-t1;
disp(['Average time of Batch after 100 times training is : ' num2str(time/runningcycles)]);